...
- **Lead agent for Decision Review during Init, Discover, Define, and Idle** (`/pdlc decision`): When a decision is issued during Oracle's lead phases (or when no phase is active), Oracle orchestrates the Decision Review Party — convening all agents, facilitating discussion, writing the MOM, and driving reconciliation
