...
> An opinionated Claude Code plugin that carries a feature from raw idea to shipped, production-ready release through a single integrated lifecycle.
...
| Capability | What PDLC Provides |
|------------|---------------------|
| **Methodology discipline** | TDD, systematic debugging, subagent reviews, worktrees |
| **Specialist team** | Named agent roles, sprint workflow, real browser automation, safety guardrails |
| **Context integrity** | Context-rot prevention, spec-driven execution, file-based state, multi-agent waves |
| **Rigorous reasoning** | Adversarial review, edge case analysis, divergent ideation, multi-agent party mode |
...
- **TDD**: Enforced — Claude must write failing tests before any implementation code; no exceptions without explicit human override
- **Execution model**: Sequential with fast switching — Claude works through `bd ready` queue by priority, one task at a time
  - When a task is claimed, **human chooses execution mode**:
    - **(Agent Teams)** — multiple specialist agents collaborate on the task (e.g. backend agent + frontend agent + QA agent working in concert)
    - **(Sub-Agent)** — single focused subagent handles the task end-to-end
...
- **Test layers** (all active by default; any layer can be skipped via human instruction or Constitution config):
  ...
  2. Integration tests
  3. E2E tests — **real Chromium instance**, not a simulator
...
- **Context bar** — color-coded progress bar with calibrated scaling:
  - Green `<50%` used
  - Yellow `50–65%`
  - Orange `65–80%`
