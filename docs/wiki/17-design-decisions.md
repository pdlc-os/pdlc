...
### Pitch Round + Vote between cross-talk and human escalation

Cross-talk gives agents three rounds of dialogue to converge. When cross-talk fails (locked positions or 3 rounds without convergence), the natural-feeling next step is to surface the disagreement to the human. But that loses two pieces of information: each disagreeing agent's *closing argument* given the points cross-talk surfaced, and the team's overall sentiment quantified as a vote. Pitch+vote captures both before any human escalation. Each disagreeing agent delivers a 1–3 paragraph closing argument; all meeting participants vote (abstention valid); the lead applies a threshold rule — supermajority (≥66.7%) binds the lead, simple majority (>50% but <66.7%) is strongly considered with override-rationale required, no majority is the lead's discretion. The lead recuses from both voting and the threshold authority if they're themselves a party to the dispute, in which case the vote tally goes to the human as input. Pitch+vote does not apply to Tier 1 hard blocks (security/safety blocks cannot be voted out), nor in a binding way to the Threat Modeling Party at Step 10.5 or the Design-Laws Roundtable at Step 10.6 (the human owns final acceptance at Step 12 by design; P0 UX findings from Step 10.6 are non-negotiable on the same basis as Tier 1 hard blocks). The mechanism reduces unnecessary human escalation when the team has a clear lean, and it gives the lead a defensible authority structure when they have to decide — supermajority means the lead is bound, no majority means they're unbound, and overrides at simple majority are recorded in the MOM. The receipt is durable and auditable.

### Tier-aliased model declarations in agent frontmatter

Every agent's `model:` frontmatter field uses **tier aliases** (`opus` / `sonnet` / `haiku`) rather than fully-qualified model IDs (`claude-opus-4-7`, `claude-sonnet-4-6`, etc.). Tier aliases are documented in Claude Code's harness as first-class values for the `model:` field; they resolve to the current latest model in that tier at runtime. Pinning a specific version means every Anthropic model release silently makes PDLC stale until the next PDLC release catches up — and we proved this when v2.16.0 found 5 of 10 agents still pointing at Opus 4.6 long after Opus 4.7 had shipped. Tier aliases let agents move forward with the platform automatically. We keep PDLC's intentional tier choice per agent role (Opus for Bolt/Friday/Neo/Atlas/Pulse — leadership and complex-reasoning roles; Sonnet for Echo/Jarvis/Muse/Phantom — focused specialist work) since that's a deliberate cost/quality calibration. Specific-version pinning is reserved for the rare case where reproducibility (compliance, regression testing) requires it; the override path is `CLAUDE_CODE_SUBAGENT_MODEL` at the session level.
...
### Design-laws audit integrated into Brainstorm Design as a Muse-led roundtable

UX-quality reviews historically happen either too early (a design system review with no actual feature behind it) or too late (a usability test on shipped product with no time left to revise). PDLC integrates UX-quality review into Brainstorm Design at Step 10.6, immediately after threat-modeling and before the design approval gate. The placement parallels the threat-modeling rationale: design artifacts are concrete enough to audit, but the design isn't locked, so findings can drive revisions before approval. Muse takes lead with explicit Neo→Muse and Muse→Neo handoffs; the human reviews the resulting `ux-review.md` alongside the other four design documents at Step 12. The audit uses PDLC's existing party-mode + cross-talk machinery, plus the operationalized methodology in `agents/extensions/muse-ux-design.md` (Nielsen 10 heuristics, 8-state coverage matrix, cognitive-load 8-item checklist, anti-pattern refuse list). A triage gate (3 questions) decides depth: Skip (no UI surface), Lite (Muse solo — anti-pattern + UX-writing + 8-state spot-check), or Full (Design-Laws Roundtable). Severity tags P0/P1/P2/P3 — P0 findings block ship on the same basis as Tier 1 security/safety blocks; P1-P3 can be accepted-as-tradeoff with an ADR. The resulting scorecard (Nielsen total, cognitive-load failure count, finding counts) is the *initial* value carried into Construction's UX Review (where the as-built scorecard is captured) and Reflect's METRICS trend analysis. The depth tiers prevent the lean-process tax for non-UI features and let the full team converge on UX-heavy ones.
