...
**Question budget.** The brainstorm phase is intentionally bounded so the human is asked roughly 25–35 questions end-to-end (down from ~55–65 in earlier versions). Specifically: Socratic interview = 3 rounds × max 4 questions, Adversarial Review = top 3 follow-ups, Edge Case Analysis = max 3 prompts (1 triage + up to 2 follow-ups), UX Discovery = max 3 visual-first questions (only on UI/UX features when the visual companion is available — otherwise skipped with a logged reason), Bloom's = 3 rounds × max 3 questions plus 1 synthesis check. Approval gates (PRD, design, plan) and the build-start prompt add a small fixed overhead. Anything not surfaced by these bounded prompts is captured downstream in the PRD's known-risks section or as flagged items in the design doc — the user can refine those during review without dedicated questioning.
